...
public class DashboardController {
    ...
    @FXML private Label issuedBagsLabel;
    ...
    private void refreshDashboard() {
        ...
        Map<String, Long> issuedCounts = calculateIssuedCounts(inventory);
        ...
        issuedBagsLabel.setText(String.valueOf(issuedCounts.values().stream().mapToLong(Long::longValue).sum()));
        ...
        populateStockChart(availableCounts, issuedCounts);
    ...
    private Map<String, Long> calculateIssuedCounts(List<BloodBagRecord> inventory) {
        Map<String, Long> counts = new LinkedHashMap<>();
        for (String bloodType : BLOOD_TYPES) {
            counts.put(bloodType, 0L);
        }
        for (BloodBagRecord record : inventory) {
            if ("ISSUED".equalsIgnoreCase(record.getEffectiveStatus())) {
                counts.put(record.getBloodType(), counts.getOrDefault(record.getBloodType(), 0L) + 1);
            }
        }
        return counts;
    ...
    private void populateStockChart(Map<String, Long> availableCounts, Map<String, Long> issuedCounts) {
        long availableTotal = availableCounts.values().stream().mapToLong(Long::longValue).sum();
        long issuedTotal = issuedCounts.values().stream().mapToLong(Long::longValue).sum();

        XYChart.Series<String, Number> availableSeries = new XYChart.Series<>();
        availableSeries.setName("Available (" + availableTotal + ")");
        XYChart.Series<String, Number> issuedSeries = new XYChart.Series<>();
        issuedSeries.setName("Issued (" + issuedTotal + ")");

        for (Map.Entry<String, Long> entry : availableCounts.entrySet()) {
            availableSeries.getData().add(new XYChart.Data<>(entry.getKey(), entry.getValue()));
        }
        for (Map.Entry<String, Long> entry : issuedCounts.entrySet()) {
            issuedSeries.getData().add(new XYChart.Data<>(entry.getKey(), entry.getValue()));
        }

        stockChart.getData().setAll(availableSeries, issuedSeries);
    }
